...
export default function ClienteLoginPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await clienteApi<{ accessToken: string }>('/auth/login', {
        method: 'POST',
        body: JSON.stringify({
  ...
  return (
    <main style={{ maxWidth: 620, margin: '0 auto' }}>
      <PageHeader title="Iniciar sesión" description="Entrá a tu cuenta Proxi con correo y contraseña." />
      <Card title="Cuenta cliente">
        <CardContent>
